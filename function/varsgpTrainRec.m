function [model, nIter, margLogL,vDiv] = varsgpTrainRec(data,model,options)
...
if options.detail == 1
    mSaveAlpha = zeros(options.maxIter,1);
...
%         model = returnAmpHyper(V,model,options);
    ...
    if options.detail == 1
        if options.inferType == 6
            mSaveAlpha(im) = model.prior.alpha;
        end
        tTmp = tic;
        mSaveTrain(im) = vargspTestSample(data.train,model,options);
        nEstTrain = nEstTrain+toc(tTmp);
        mSaveTime(im) = toc(tstart)-nEstTrain;
    end
...
if options.detail == 1 && options.start == 1
    inferType = options.inferType;
    if options.inferType == 6 && options.constraint == 1
        inferType = 7;
    end
    save(['Rec',num2str(data.dataSource),'Size',num2str(data.U),'Infer',...
        num2str(inferType),'Cluster',num2str(options.cluster),'Model.mat'],...
        'mSaveAlpha','mSaveTrain','mSaveTime');
